                ...
                if (merged_col[i]==0){ // left boundary
                    write_rule_loop0: for (int j=0; j<3; j++){
                        if (input_index[j*COL_SIZE+i]!=-1){ // must be first group
                            k_temp[write_num] = (j*3+1);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = prevIdx;
                            write_num++;
                            k_temp[write_num] = (j*3);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = prevIdx+1;
                            write_num++;
                        }
                    }
                    ...
                    write_rule_loop1: for (int j=0; j<3; j++){
                        if (input_index[j*COL_SIZE+i]!=-1){ // must be last group
                            k_temp[write_num] = (j*3+2);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = currIdx-2;
                            write_num++;
                            k_temp[write_num] = (j*3+1);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = currIdx-1;
                            write_num++;
                        }
                    }
                } else {
                    write_rule_loop2: for (int j=0; j<3; j++){
                        if (input_index[j*COL_SIZE+i]!=-1){
                            ...
                            k_temp[write_num] = (j*3+2);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = prevIdx+outIdx-1;
                            write_num++;
                            k_temp[write_num] = (j*3+1);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = prevIdx+outIdx;
                            write_num++;
                            k_temp[write_num] = (j*3);
                            i_temp[write_num] = input_index[j*COL_SIZE+i];
                            o_temp[write_num] = prevIdx+outIdx+1;
...
void generate_out(hls::stream<hls::vector<DTYPE,COL_SIZE*3>> &KStream, // k,i,o
                  hls::stream<hls::vector<DTYPE,COL_SIZE*3>> &IStream, // k,i,o
                  hls::stream<hls::vector<DTYPE,COL_SIZE*3>> &OStream, // k,i,o
                  hls::stream<DTYPE> &RuleWriteLength,
    ...
    int rule_size;
    for(int r=0; r<ROW_SIZE; r++){
        rule_size = RuleWriteLength.read();
        hls::vector<DTYPE,COL_SIZE*3> k = KStream.read();
        hls::vector<DTYPE,COL_SIZE*3> i = IStream.read();
        hls::vector<DTYPE,COL_SIZE*3> o = OStream.read();
        for (int ii=0; ii<COL_SIZE*3; ii++){
            if (ii<rule_size){
                hls::vector<DTYPE,3> temp;
                temp[0] = k[ii];
                temp[1] = i[ii];
                temp[2] = o[ii];
                out_Rule[start+ii] = temp;
            }
        }
        start += rule_size;
    }
    for (int iii=start; iii<KERNEL_SIZE*KERNEL_SIZE*NUM_FEATURE*3; iii++){
        out_Rule[iii] = -1; // for done
    }
}
