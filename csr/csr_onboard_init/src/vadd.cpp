...
#pragma HLS INTERFACE m_axi port = out_Rule bundle = gmem2 depth=600
    ...
    load_row_data: for (int i=0; i<=ROW_SIZE; i++){
        csr_row[i] = in_csr_row[i];
    } 
    load_col_data: for (int i=0; i<NUM_FEATURE; i++){
        csr_col[i] = in_csr_col[i];
    } 
    init_rule: for (int k=0; k<KERNEL_SIZE*KERNEL_SIZE*2*NUM_FEATURE; k++){
        Rule[k] = -1;
    }
    // CSR format test data (simple example)
    int numFeaturePerRow[ROW_SIZE]; // initialize
    cal_num_feature: for (int i=0; i<ROW_SIZE; i++){
                ...
                if (merged_col[i]==0){ // left boundary
                    write_rule_loop0: for (int j=0; j<3; j++){
                        if (input_index[j][i]!=-1){ // must be first group
                            ...
                            Rule[(j*3+1)*RK+RO+rule_counter[j*3+1]++] = out_csr_row[r];
                            Rule[(j*3)*RK+rule_counter[j*3]] = input_index[j][i];
                            Rule[(j*3)*RK+RO+rule_counter[j*3]++] = out_csr_row[r]+1;
                        }
                    }
                    ...
                    write_rule_loop1: for (int j=0; j<3; j++){
                        if (input_index[j][i]!=-1){ // must be last group
                            ...
                            Rule[(j*3+2)*RK+RO+rule_counter[j*3+2]++] = out_csr_row[r+1]-2;
                            Rule[(j*3+1)*RK+rule_counter[j*3+1]] = input_index[j][i];
                            Rule[(j*3+1)*RK+RO+rule_counter[j*3+1]++] = out_csr_row[r+1]-1;
                        }
                    }
                } else {
    ...
    write_out_rule: for (int k=0; k<KERNEL_SIZE*KERNEL_SIZE*2*NUM_FEATURE; k++){
        out_Rule[k] = Rule[k];
    }
