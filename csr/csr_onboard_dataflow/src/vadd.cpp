...
void readCsrData(hls::stream<hls::vector<DTYPE,(ROW_SIZE+1)>> &CsrRow,
    ...
    // Make stream
    CsrRow.write(csr_row);
    CsrRow_.write(csr_row);
    CsrCol.write(csr_col);
}
...
#pragma HLS DATAFLOW
    int out_rule_temp[KERNEL_SIZE*KERNEL_SIZE*2*NUM_FEATURE];
    for (int loop=0; loop<(KERNEL_SIZE*KERNEL_SIZE*2*NUM_FEATURE); loop++){
        out_rule_temp[loop] = -1;
    }
    // type conversion
    ...
    */
    int rule_size;
    rule_size = write_rule(RuleStream, MergedCol_, KIPairs, OutNumFeaturePerRow, OutCsrCol);

    hls::vector<DTYPE,KERNEL_SIZE*KERNEL_SIZE> rule_counter;
    ...
    int RO = NUM_FEATURE;
    for (int loop=0; loop<rule_size; loop++){
        hls::vector<DTYPE,3> Rule = RuleStream.read();
        int k = Rule[0];
        int i = Rule[1];
        int o = Rule[2];
        out_rule_temp[k*RK+rule_counter[k]] = i;
        out_rule_temp[k*RK+RO+rule_counter[k]++] = o;
    }

    // copy generated data
    for (int i=0; i<(KERNEL_SIZE*KERNEL_SIZE*2*NUM_FEATURE); i++){
        out_Rule[i] = out_rule_temp[i];
    }
}
